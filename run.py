...
def install_mbedtls(version, source_dir, build_dir, install_dir, cmake_args):
    ...
    git_clone_shallow('https://github.com/Mbed-TLS/mbedtls.git', version, os.path.join(source_dir, 'mbedtls'))
    with cd(os.path.join(source_dir, 'mbedtls')):
        ...
        cmd(['cmake',
            f"-DCMAKE_INSTALL_PREFIX={cmake_path(os.path.join(install_dir, 'mbedtls'))}",
             ...
             '-B', os.path.join(build_dir, 'mbedtls')] + cmake_args)
        cmd(['cmake', '--build', os.path.join(build_dir, 'mbedtls'),
...
def install_libjpeg_turbo(version, source_dir, build_dir, install_dir):
    ...
    git_clone_shallow('https://github.com/libjpeg-turbo/libjpeg-turbo.git',
                      version, os.path.join(source_dir, 'libjpeg-turbo'))
...
def install_libyuv(version, source_dir, build_dir, install_dir):
    ...
    git_clone_shallow('https://chromium.googlesource.com/libyuv/libyuv', version, os.path.join(source_dir, 'libyuv'))
    mkdir_p(os.path.join(build_dir, 'libyuv'))
    with cd(os.path.join(build_dir, 'libyuv')):
        cmd(['cmake', os.path.join(source_dir, 'libyuv'),
...
def install_deps(target_platform: str, build_platform: str, source_dir, shared_source_dir, build_dir, install_dir,
                 debug):
    with cd(BASE_DIR):
        ...
        if not os.path.exists(os.path.join(dir, '.git')):
            ...
            with cd(dir):
                ...
                download('https://github.com/cisco/libsrtp/commit/91ceb8176afdbc5f2bdde0e409da011e99e22d9c.diff')
...
def main():
    ...
    install_deps(target_platform, build_platform, source_dir, shared_source_dir, build_dir, install_dir, args.debug)
    ...
    with cd(sorac_build_dir):
        ...
        # NvCodec
        if target_platform in ('windows_x86_64', 'ubuntu-20.04_x86_64', 'ubuntu-22.04_x86_64'):
        ...
        if target_platform == 'ios':
            cmd(['cmake', '--build', '.', f'-j{multiprocessing.cpu_count()}', '--config', configuration,
                '--target', 'sorac', '--', '-arch', 'x86_64', '-sdk', 'iphonesimulator'])
            cmd(['cmake', '--build', '.', f'-j{multiprocessing.cpu_count()}', '--config', configuration,
                '--target', 'sorac', '--', '-arch', 'arm64', '-sdk', 'iphoneos'])
            # 後でライブラリは差し替えるけど、他のデータをコピーするためにとりあえず install は呼んでおく
            cmd(['cmake', '--install', '.'])
            cmd(['lipo', '-create', '-output', os.path.join(build_dir, 'sorac', 'libsorac.a'),
                os.path.join(build_dir, 'sorac', f'{configuration}-iphonesimulator', 'libsorac.a'),
                os.path.join(build_dir, 'sorac', f'{configuration}-iphoneos', 'libsorac.a')])
            shutil.copyfile(os.path.join(build_dir, 'sorac', 'libsorac.a'),
        ...
        if target_platform in ('windows_x86_64',):
            shutil.copyfile(os.path.join(sorac_build_dir, 'bundled', 'sorac.lib'),
                            os.path.join(install_dir, 'sora', 'lib', 'sorac.lib'))
        else:
            shutil.copyfile(os.path.join(sorac_build_dir, 'bundled', 'libsorac.a'),
                            os.path.join(install_dir, 'sorac', 'lib', 'libsorac.a'))
    ...
    if args.package:
        ...
        ext = 'zip' if target_platform in ('windows_x86_64', 'windows_arm64') else 'tar.gz'
